...
# --- Проверка root ---
if [[ $EUID -ne 0 ]]; then
    err "Запустите с sudo: sudo bash scripts/install_debian.sh"
fi
...
if ! id -u accos &>/dev/null; then
    useradd -m -s /bin/bash -d /opt/accos accos
    ok "Пользователь accos создан"
else
    ok "Пользователь accos уже существует"
fi

# --- 3. Настройка проекта ---
log "[3/9] Настройка проекта..."
if [[ "$PROJECT_ROOT" != "/opt/accos" ]]; then
    warn "Проект находится в $PROJECT_ROOT, а не в /opt/accos"
    warn "Рекомендуется скопировать проект в /opt/accos или создать симлинк"
    echo -n "Создать симлинк /opt/accos -> $PROJECT_ROOT? [y/N]: "
    read -r symlink_choice
    if [[ "$symlink_choice" =~ ^[yY] ]]; then
        ln -sfn "$PROJECT_ROOT" /opt/accos
        PROJECT_ROOT="/opt/accos"
        ok "Симлинк создан"
    fi
fi
...
if [[ ! -d "$PROJECT_ROOT/.venv" ]]; then
    su - accos -c "cd '$PROJECT_ROOT' && python3 -m venv .venv"
    ok ".venv создан"
else
...
DB_NAME="${DB_NAME:-accos}"
DB_USER="${DB_USER:-accos}"
DB_PASS="${DB_PASS:-}"

if [[ -z "$DB_PASS" ]]; then
    DB_PASS=$(openssl rand -base64 24)
    warn "Сгенерирован пароль БД: $DB_PASS (сохраните его!)"
fi
...
echo -n "Введите IP для доступа к ACCOS (Enter для $SERVER_IP): "
read -r CUSTOM_IP
CUSTOM_IP="${CUSTOM_IP:-$SERVER_IP}"

echo -n "Введите порт для ACCOS (Enter для 8000): "
read -r APP_PORT
APP_PORT="${APP_PORT:-8000}"

echo -n "Введите порт для Nginx (Enter для 80): "
read -r NGINX_PORT
...
if [[ ! -f "$ENV_FILE" ]]; then
    ...
    ok "Создан config/.env из .env.example"
fi
...
echo -n "Включить HTTPS с самоподписанным сертификатом? [y/N]: "
read -r USE_HTTPS
SSL_CONFIG=""
...
server {
    ...
    # Статика (сгенерированные изображения, загрузки)
    location /static/ {
...
systemctl enable accos.service
...
# --- 12. Статические директории ---
mkdir -p "$PROJECT_ROOT/static/generated" "$PROJECT_ROOT/static/uploads" "$PROJECT_ROOT/backend/logs"
chown -R accos:accos "$PROJECT_ROOT/static" "$PROJECT_ROOT/backend/logs"
ok "Директории созданы"

# --- Завершение ---
systemctl start accos.service
sleep 2
if systemctl is-active --quiet accos.service; then
    ok "ACCOS сервис запущен"
else
    warn "Сервис не запустился. Проверьте: journalctl -u accos.service"
fi
