...
```bash
# Полное удаление ACCOS с сервера
sudo bash scripts/uninstall_debian.sh
```

Скрипт удаляет:
- systemd сервис `accos.service`
- Nginx конфигурацию и SSL-сертификаты
- Базу данных PostgreSQL (accos) и пользователя БД
- Файлы проекта (`.venv`, `static/`, `dist/`, `node_modules/`, `logs/`)
- Пользователя `accos`

Для Windows удалите службу NSSM и папку проекта вручную.
