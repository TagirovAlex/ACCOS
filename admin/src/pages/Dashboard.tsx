...
const StatCard = ({ icon, label, value, gradient }: StatCardProps) => (
  <Grid size={{ xs: 12, sm: 6, md: 4 }}>
    <Card sx={{ position: "relative", overflow: "hidden", border: "none", borderRadius: 3 }}>
      <Box sx={{
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2.5} mb={4}>
        <StatCard icon={<PeopleIcon fontSize="large" />} label="Пользователи" value={users ?? "..."} gradient="linear-gradient(135deg, #448aff 0%, #7c4dff 100%)" />
        <StatCard icon={<GroupIcon fontSize="large" />} label="Группы доступа" value={groups ?? "..."} gradient="linear-gradient(135deg, #43a047 0%, #00bfa5 100%)" />
        <StatCard icon={<ChatIcon fontSize="large" />} label="Чаты" value={chats ?? "..."} gradient="linear-gradient(135deg, #ffa726 0%, #ff7043 100%)" />
        <StatCard icon={<AutoAwesomeIcon fontSize="large" />} label="Генерации" value={generations ?? "..."} gradient="linear-gradient(135deg, #ab47bc 0%, #e040fb 100%)" />
        <StatCard icon={<ImageIcon fontSize="large" />} label="Ресурсы (изображения)" value={assets ?? "..."} gradient="linear-gradient(135deg, #26c6da 0%, #448aff 100%)" />
      </Grid>
